...
"    return dot(abs(c1 - c2), half3(0.26, 0.55, 0.19));\n"
"}\n"
...
"    if (wd(f_, h) < wd(e, i) * 0.95) {\n"
"        half3 target = (d_ef <= d_eh) ? f_ : h;\n"
"        float blend = saturate((total_dist - 0.3) * 6.0);\n"
"        if (f.x > 0.5 && f.y > 0.5) res = lerp(res, target, blend * blend);\n"
"    }\n"
"    if (wd(d, b) < wd(e, a) * 0.95) {\n"
"        half3 target = (d_ed <= d_eb) ? d : b;\n"
"        float blend = saturate((total_dist - 0.3) * 6.0);\n"
"        if (f.x < 0.5 && f.y < 0.5) res = lerp(res, target, blend * blend);\n"
"    }\n"
"    if (wd(b, f_) < wd(e, c) * 0.95) {\n"
"        half3 target = (d_eb <= d_ef) ? b : f_;\n"
"        float blend = saturate((total_dist - 0.3) * 6.0);\n"
"        if (f.x > 0.5 && f.y < 0.5) res = lerp(res, target, blend * blend);\n"
"    }\n"
"    if (wd(h, d) < wd(e, g) * 0.95) {\n"
